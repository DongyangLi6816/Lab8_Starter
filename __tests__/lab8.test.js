describe('Basic user flow for Website', () => {
  ...
  it('Make sure <product-item> elements are populated', async () => {
    ...
    for (let i = 0; i < prodItems.length; i++) {
      ...
      let text = await data.jsonValue();

      expect(text.title.length).not.toBe(0);
      expect(text.price.length).not.toBe(0);
      expect(text.image.length).not.toBe(0);
    }
  ...
  it('Checking number of items in cart on screen', async () => {
    console.log('Checking number of items in cart on screen after adding all items...');
